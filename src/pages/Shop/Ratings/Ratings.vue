...
export default {
  data() {
    return {
      // 是否只显示有内容评论的
      onlyText: true,
      // selectType:有三种情况-->1:吐槽 2:全部 0:推荐
      selectType: 1
    };
  },
  components: {
  ...
  computed: {
    ...
    filterRatings() {
      ...
      // 此时,ratings数组包含全部的,无论是吐槽类型
      // 过滤ratings数组
      return ratings.filter(rating => {
        // 从遍历的过后的每一个rating对象中拿到rateType(评论类型) 和 text(评论文本)
        // // rateType有两种情况 0/1
        const { rateType, text } = rating;
        // 判断当前点击selectType的类型,当selectType === 2的时候,selectType的类型是全部
        return (
  ...
  methods: {
    // 当调用setSelectType这个函数的时候,传入type
    setSelectType(type){
      this.selectType = type
    },
    // 调用该函数,是否显示 --> 只显示有内容评论的
    toggleOnlyText(){
