<template>
  <div>
      ...
      <div class="content">
        <div class="content-left">
          <div class="logo-wrapper">
            <div class="logo" :class="{highlight:totalCount>0}" @click="showCard">
    ...
    <!-- 遮罩层 -->
    <div class="list-mask" v-show="showList" @click="showCard"></div>
  </div>
</template>
...
export default {
  ...
  computed: {
    // 从state对象中拿到foodArr和info对象
    ...mapState({
      foodArr: state => state.shop.foodArr,
      info: state => state.shop.info
    }),
    // 拿到总数量和总价格
    ...mapGetters(["totalCount", "totalPrice"]),
    // 根据总数量和总价格之间的关系-->确定显示相对应的类名
    payClass() {
      // 拿到总价格和起送的最低价格
    ...
    showList() {
      ...
      this.$nextTick(() => {
        ...
        if (!this.scroll) {
          this.scroll = new BScroll(".list-content", {
            // 在计算属性中,计算属性相关属性发生变化,就会new一个滑动对象
            click: true
          });
        }else{
          //使用单例模式出现问题;
      ...
      // 当totalCount不是0的时候直接return-->isShowCard的状态值
